...
- [ ] **Lock the twin contract before picking a dojo** (2026-09-25): CAD→URDF/USD or MJCF lockstep, observation parity, motorcycle-crown tire contact, firmware zeros, stack willingness. Pipeline (Isaac / MuJoCo / mjlab / other) stays **TBD**. Living-drawings 3D sandbox is a design toy, not this twin ([`sim-sandbox.md`](sim-sandbox.md)).
- [ ] Twin fidelity first — masses, inertias, wheel contact (crown + compliance, not a flat-shoulder cylinder), hip-roll plant — before domain randomization. Observations must match **real sensors** (SpdrBot: sim-only velocity trained a policy the Pico could not run). Watch reward-hacking (their dinosaur gait).
