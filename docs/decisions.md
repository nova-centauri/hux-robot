...
| Power | **All-electric.** **6S** pack class (LiPo preferred; high-drain Li-ion 21700 acceptable) — **~22.2 V nominal / 25.2 V full**. One large pack **or two in parallel** (matched, fused). **Regulated step-down** for 5 V / 12 V logic and pose rails. TBS Nano RX. **Superseded 4S on 2026-09-26.** |
...
| Real-time board | **Not the F765-Wing** (no CAN). It is the **P0–P1 bench board** only. P2+ board must have CAN: **Teensy 4.1-class** or **H743-WING-class** — pick with the actuators. Not bought. |
...
- **Power: 6S** class, ~22.2 V nominal / 25.2 V full. LiPo preferred; high-drain Li-ion 21700 is the acceptable "similar." One pack or two in parallel (matched before paralleling, fused per pack). Capacity / C / SKU still TBD. Rails: **5 V** (MCU, RX, Pi), **12–19 V** (companion slot; a Jetson kit needs this — 6S full exceeds its 19 V input), pose rail if servos. Motor bus is 6S direct.
...
- **Real-time MCU with CAN** runs the core at 1 kHz with IMU, CRSF, hardware watchdog and torque cut. **F765-Wing is P0–P1 bench learning only** (CRSF, one SimpleFOC wheel). Candidates: Teensy 4.1-class (3× CAN FD) or H743-WING-class (1× CAN, dual IMU, SD). Choose with the actuators. Not bought.
