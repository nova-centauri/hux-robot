...
| Actuator shortlist (not ordered) | **Recommended set (2026-09-26): 4× RobStride 02** (knees + hip roll), **2× RobStride 00** (hip swing), **2× RobStride 05** (wheels), with the **hip roll axes brought in to ≤ 3.5" from the centreline** so the one-wheel hold is 5.4 N·m against RS02's 7 rated (one-leg-stance study: 8.3 N·m at the drawn 5.4"). If the layout cannot get the hips in, roll becomes RS06 (+240 g each). **First buy: one RS02** on the Teensy. Steve picks; then it is a cart line. Classic CAN 1 Mbit on **two buses** (wheels + roll; knees + swing). Requires **≥24 V bus → 8S**. GIM8108-8 was the earlier yardstick. [`research/actuator-shortlist.md`](research/actuator-shortlist.md). Steve confirms; then it is a cart line. |
