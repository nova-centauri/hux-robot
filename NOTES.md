...
- [ ] **First printable wheel-leg** for a **~9.5"** step (**Phase D**). Keep the V1 mass story in view (**under 6 lb** / aim 4–5 lb). FC stays TBD; do not block the print on electronics. Do not start this to skip research. Do not buy a wheel motor to fill a class.
...
- No spend until Steve asks. Prefer parts already on hand. Do not buy a gimbal motor, FOC board, encoder, or “better” hub to fill a class.
- Do not lock an FC in docs to make the repo look finished.
- Do not lock a wheel-motor SKU. Classes only. Do not pick ODrive 63xx / hoverboard hubs for a 4–6 lb Hux.
...
- Requirements: [`docs/requirements.md`](docs/requirements.md) — R24 under 6 lb (aim 4–5 lb) · R25 wheel motors = torque bandwidth, not continuous power
- Electronics wheel drive: [`docs/electronics.md`](docs/electronics.md) — gimbal 2208–4108 / small outrunner+reduction; reuse FOC (R18); no SKU
- Mechanical mass sketch: [`docs/mechanical.md`](docs/mechanical.md) — TBD budget lines + ~0.8 Nm order-of-magnitude
